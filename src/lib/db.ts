...
export async function seedInitialDataIfNeeded(force = false) {
  ...
  // 2. Seed Camino 2026 if needed (or upgrade to 6k test dataset)
  const caminoExpenses = await db.expenses.where('tripId').equals('trip_camino_2026').toArray();
  const caminoTotal = caminoExpenses.reduce((sum, e) => sum + e.originalAmount, 0);

  if (!existingCamino || caminoTotal < 5000 || force) {
    ...
    // Clear old camino expenses and load fresh 6k dataset
    await db.expenses.where('tripId').equals('trip_camino_2026').delete();
